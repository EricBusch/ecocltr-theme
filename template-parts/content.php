<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="relative pt-16 before:absolute before:top-0 before:left-0 before:h-px before:w-6 before:bg-zinc-950 after:absolute after:top-0 after:right-0 after:left-8 after:h-px after:bg-zinc-950/10">
		<div class="relative lg:-mx-4 lg:flex lg:justify-end">
			...
			<h2 class="text-2xl font-semibold text-zinc-950"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>
			<dl class="lg:absolute lg:top-0 lg:left-0 lg:w-1/3 lg:px-4">
				<dt class="sr-only"><?php esc_html_e( 'Published', 'ecocltr' ); ?></dt>
				...
				<dt class="sr-only"><?php esc_html_e( 'Author', 'ecocltr' ); ?></dt>
			...
			<a class="mt-8 inline-flex rounded-full bg-zinc-950 px-4 py-1.5 text-sm font-semibold text-white transition hover:bg-zinc-800" aria-label="<?php echo esc_attr( sprintf( /* translators: %s: Post title */ __( 'Read more: %s', 'ecocltr' ), get_the_title() ) ); ?>" href="<?php the_permalink(); ?>">
				<?php esc_html_e( 'Read more', 'ecocltr' ); ?>
			</a>
		</div>
	</div>
</article>
